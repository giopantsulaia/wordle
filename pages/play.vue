...
function styles(counter,index) {
    const includes = (words.value.length > counter && correctWord.includes(words.value[counter][index]));
    const correct = (words.value.length > counter && words.value[counter][index] === correctWord[index]);
    const wrong = (words.value.length > counter && !correctWord.includes(words.value[counter][index]));
    return [correct ? 'bg-green-600' : includes ? 'bg-orange-500' : wrong ? 'bg-red-500' : 'bg-white',{'text-gray-700' : !words.value[counter]}]
}
    ...
    <section>
        ...
        <main class="mt-16">
            <div class="flex flex-wrap w-[400px]  m-auto gap-4 justify-center">
                <div class="w-16 h-16 flex items-center rounded-lg border"
                    v-for="(box, index) in 5">
                    <p class="w-full h-full text-4xl font-black uppercase text-center pt-2 text-white rounded-lg border-2 border-gray-900"
            ...
            <div class="w-[700px] m-auto mt-28 flex flex-wrap justify-center">
                <button v-for="letter in keys" :key="letter" @click="handleClickLetter(letter)"
                    class="m-2 border-2 border-gray-500 w-[53px] h-14 uppercase text-xl font-black rounded"
                    :class="{'ml-6' : letter === 'a'}">
                    {{ letter }}
                </button>
                <button class="w-20 m-2 h-14 border-2 border-gray-500 rounded text-xl font-black" @click="deleteLetter">⌫</button>
            </div>
            <button class="absolute top-2 left-4 h-4  border-x-2 border-gray-600 rounded-lg p-6 items-center flex text-2xl" @click="startOver">Retry</button>
        </main>
    </section>
</template>
